 ...
 *
 * @constructor Creates an EqualOperation that determines if the left value is
...
class EqualOperation(leftValue: Value?, rightValue: Value?) : EqualityOperation(leftValue, rightValue)
{
    ...
    /**
     * Determines if the left and right values are equal.
     *
     ...
     *
     * @return A BooleanValue holding true if the two values are equal, or a
     *      BooleanValue holding false if the two values are not equal.
     */
    ...
    companion object CREATOR : Parcelable.Creator<EqualOperation>
    {
        private val TAG: String = EqualOperation::class.java.simpleName

        override fun createFromParcel(parcel: Parcel): EqualOperation
        {
            return EqualOperation(parcel)
        }

        override fun newArray(size: Int): Array<EqualOperation?>
        {
