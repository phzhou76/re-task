 ...
 *
 * @constructor Creates a GreaterThanOperation that determines if the left value
 *      is greater than the right value.
 */
class GreaterThanOperation(leftValue: NumericValue?, rightValue: NumericValue?)
    : NumericEqualityOperation(leftValue, rightValue)
{
     ...
     *
     * @return A BooleanValue holding true if the left value is strictly greater
     *      than the right value, or false if the left value is not strictly greater
    ...
    companion object CREATOR : Parcelable.Creator<GreaterThanOperation>
    {
        private val TAG: String = GreaterThanOperation::class.java.simpleName

        override fun createFromParcel(parcel: Parcel): GreaterThanOperation
        {
            return GreaterThanOperation(parcel)
        }

        override fun newArray(size: Int): Array<GreaterThanOperation?>
        {
